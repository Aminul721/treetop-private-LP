...
$.fn.percircle = function (options) {
  ...
  return this.each(function () {
    ...
    if (percent || localOptions.displayTextAtZero) {
      ...
      setTimeout(function () {
        $('.bar', percircle).css({
          '-webkit-transform': 'rotate(' + rotationDegrees * 12 + 'deg)',
          '-moz-transform': 'rotate(' + rotationDegrees * 12 + 'deg)',
          '-ms-transform': 'rotate(' + rotationDegrees * 12 + 'deg)',
          '-o-transform': 'rotate(' + rotationDegrees * 12 + 'deg)',
          'transform': 'rotate(' + rotationDegrees * 12 + 'deg)'
        });
      }, 0);
...
var getCountdown = function (percircle, options, progressBarStyle) {
  ...
  function timer() {
    ...
    if (secs > 3) percircle.addClass('gt50');
    if (secs < 3) percircle.removeClass('gt50');
